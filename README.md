...
- **港股代码统一支持 (回测/区间/财报/详情页均可用)**: `data_service.resolve_code` 支持港股 —
  显式 `.HK` 后缀 (如 `00700.HK`) 或 **4~5 位纯数字** (如 `0700`/`00700` → `00700.HK`;
  A股 6 位代码不受影响, 如 `000858` → `000858.SZ`)。策略回测、区间交易估价、财报挖掘
  输入港股代码即可运行: 日线走腾讯港股 K 线; 财报挖掘走东财港股数据 (报告标注
  "东方财富港股...金额单位亿港元", `source=hk_eastmoney`)。
  注意港股无可靠前复权序列, 回测/收益用原始价 (与 A 股 qfq 口径不同)。
- **个股详情页支持港股**: 详情页 (stock_detail.html) 输入港股代码 (如 `00700.HK`) 即按港股
  渲染 — 头部「港股」标签、PB/PE、每股分红 (港元)、股息率、52周高低、总股本 (股)、
  总市值 (港元)、当日行情 + K线。港股无前复权, 复权按钮仅「不复权/后复权」;
  行情/提示单位适配: 成交量「股」(港股 vol 单位是股, A股是手)、成交额「港元」
  (港股 amount 为千元口径, 与 tushare 一致)。
- **股息率筛选基于动态股息率 (TTM)**: A股与港股红利低波选股的「动态股息率 ≥/≤」筛选
  及默认排序均基于 `dividend_yield_ttm` (股息率-TTM, 当年每股分红/最新收盘价)。
- **所有输出结果均支持按名称排序**: 基本面选股 (A股/港股) 名称列点击可排序 (后端白名单
  已含 name, 中文按数据库排序规则); 自选股/每日推荐/红利低波/港股红利低波名称排序原本已支持。
