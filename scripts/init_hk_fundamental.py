"""港股基本面数据初始化脚本 (ROE 杜邦拆分)。

用法:
  python scripts/init_hk_fundamental.py                            # 全市场 2024~2025
  python scripts/init_hk_fundamental.py --industry 银行 --start 2023 --end 2025
  python scripts/init_hk_fundamental.py --codes 00700,00005 --years 2025  # 指定股票调试

说明:
  - 数据源: 东财港股财务指标/资产负债表 + 腾讯港股日线 + tushare hk_basic
  - 指标: ROE/净利润率/总资产周转率/权益乘数/毛利率/负债率/流动比率/流动资产/现金/
          存货周转天数/应收周转天数/EPS/营收/净利/总市值 (金额单位为万港元)
  - ROE 杜邦拆分: roe ≈ 净利润率 × 总资产周转率 × 权益乘数 (归母口径, 内部一致)
  - 幂等 upsert (按 ts_code+year 唯一), 可重复运行续跑
  - 表格: hk_fundamental_screen
"""
import argparse
import sys
import time
...
async def main() -> None:
    parser = argparse.ArgumentParser(description="港股基本面数据初始化")
    ...
    if codes:
        ...
        for ts_code in codes:
            try:
                ...
                for y in years:
                    r = await hkf.compute_stock_row(m, y)
                    if r is not None:
                        rows.append(r)
    ...
    t0 = time.time()
    result = await hkf.sync_industry_years(args.industry, years,
