...
async def _all_stocks(limit: int = 0) -> list:
...
async def _stock_industry_map() -> dict:
...
async def _backfill_industry() -> None:
    """回填 daily_band_recommend 中行业为空的历史行 (幂等)。"""
    await pg_service.init_daily_rec_schema()
    m = await _stock_industry_map()
    await pg_service.backfill_daily_rec_industry(m)
...
async def _probe_trade_date() -> str:
    ...
    for code in ("000001.SZ", "600000.SH"):
        try:
            df = await data_service.get_daily(code, "stock", "20250101", "", adj="qfq")
...
async def scan_all(codes: str = "", industry: str = "", limit: int = 0,
    ...
    if use_cache and industry and not codes and start_date == "20170101" and not end_date:
        cur = await _probe_trade_date()
        if cur:
            existing = await pg_service.has_daily_rec(cur, industry)
            if existing:
                print(f"✓ 缓存命中: {cur} 行业[{industry}] 已有 {existing} 行, 直接读取")
                recs = await get_recommendations(cur, limit=2000, industry=industry)
                recs.update({
                    "cached": True,
                    "scanned": existing,
                    "ok": existing,
    ...
    if codes:
        ...
        stocks = await _industry_stocks(industry, limit)
    else:
        stocks = await _all_stocks(limit)
    ind_map = await _stock_industry_map()  # 总是构建: 全市场扫描也写入行业
    ...
    if skip_existing:
        cur = await _probe_trade_date()
        if cur:
            done = set(await pg_service.daily_rec_done_codes(cur))
            before = len(stocks)
    ...
    for i, (ts_code, _n) in enumerate(stocks, 1):
        try:
            ...
            r = await asyncio.to_thread(band_service.optimize_band, df, capital=100000,
        ...
        if batch and batch > 0 and len(rows) >= batch:
            stored += await pg_service.upsert_daily_rec_rows(rows)
            for r in rows:
    ...
    stored += await pg_service.upsert_daily_rec_rows(rows) if rows else 0
    # 筛选: 买入价 >= 当日收盘价 (不低于)
    for r in rows:
        if r["buy_price"] is not None and r["close"] is not None \
                and r["buy_price"] >= r["close"]:
...
async def get_recommendations(calc_date: str = "", limit: int = 500, industry: str = "") -> dict:
    """从 pgsql 查询最近计算日的推荐列表 (buy_price >= close, 按收盘价降序)。

    industry 非空时仅返回该行业 (子串匹配) 的标的。
    """
    await _backfill_industry()
    rows = await pg_service.query_daily_recommend(calc_date or None, buy_above_close=True,
                                                  limit=limit, industry=industry)
    date = rows[0]["calc_date"] if rows else await pg_service.latest_calc_date()
    return {"calc_date": date, "count": len(rows), "items": rows, "industry": industry}


async def recommend_dividend(min_dy_ttm: float = 3.0, industry: str = "",
    ...
    """
    items = await pg_service.query_dividend_recommend(
    ...
    if not year_min and not year_max:
        latest = await pg_service.latest_rlv_year()
