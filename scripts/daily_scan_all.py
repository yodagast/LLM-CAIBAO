#!/usr/bin/env python3
"""全市场每日推荐扫描脚本: 股票 + ETF 区间交易参数估算 → 入库 daily_band_recommend。

与 daily_scan.py 不同: 默认同时扫描沪深 A 股与 ETF/基金 (fund_basic), 适合每天自动跑全市场。

用法:
  python scripts/daily_scan_all.py                          # 全市场股票+ETF (约5500+, 耗时数小时, 建议后台)
  python scripts/daily_scan_all.py --limit 100              # 调试: 只算前 100 只
  python scripts/daily_scan_all.py --stocks-only            # 仅股票 (不含 ETF)
  python scripts/daily_scan_all.py --industry 银行          # 仅指定行业 (股票)
  python scripts/daily_scan_all.py --codes 000858,512170    # 指定代码 (股票/ETF 均可)
  python scripts/daily_scan_all.py --objective drawdown --max-trades 50 --sleep 0.1

说明:
  - 复用区间交易参数估算逻辑 (前复权 + T+1 + 剔除无效交易 + 交易次数上限)
  - 结果按 (calc_date, ts_code) 幂等 upsert, 可重复运行续跑/覆盖当天
  - 推荐 = 估算买入价 ≥ 当日收盘价 的公司/ETF (可在页面/API 查询)
"""
...
def main() -> None:
    _load_env()
    parser = argparse.ArgumentParser(description="全市场每日推荐扫描 (股票+ETF 区间交易参数估算入库)")
    parser.add_argument("--codes", default="", help="指定代码(逗号分隔, 股票/ETF 均可); 空=按行业或全市场")
    parser.add_argument("--industry", default="", help="行业名称(东财分类); 空=全市场")
    parser.add_argument("--limit", type=int, default=0, help="限制计算数量(0=不限, 全市场约5500只股票+ETF)")
    parser.add_argument("--stocks-only", action="store_true", help="仅扫描股票, 不含 ETF/基金")
    ...
    scope = "股票" + ("" if args.stocks_only else "+ETF")
    print(f"全市场每日推荐扫描[{scope}]: 目标={args.objective} 夏普≥{args.min_sharpe} "
    ...
    r = drs.scan_all(codes=args.codes, industry=args.industry, limit=args.limit,
                     ...
                     include_funds=not args.stocks_only)
    ...
    for it in r["items"][:20]:
        print(f"  {it['name']:<8} {it['ts_code']:<11} [{it['kind']:<4}] 收盘 {it['close']:>8.2f} "
